"""SNS #15: trajectory-budget matched broad endpoint coverage, without J* training.

Use the exact response-training IC/intervention conditions and their +/-eps
perturbations, with M common-noise pairs per condition. Noise is newly simulated:
the original response archive retains differences, not the two endpoint fields. Original normalization stays frozen.
...
def data_protocol(args):
    ...
    return dict(extra_trajectories=budget,generation_seed=GEN_SEED,batch_size=args.sim_batch,
        allocation='response-training ICs and a +/- recorded eps; M common-noise pairs; new physical noise realizations',
        ...
        response_files_sha256={n:sha(args.data_dir/n) for n in ['anchor_response.pt','response_collocation.pt']},
...
def local_conditions(args):
    meta=metadata(args);out={'x0':[],'a':[],'eps':[]}
    for filename,key in [('anchor_response.pt','anchor_response_conditions'),('response_collocation.pt','collocation_response_conditions')]:
        obj=base.safe_load(args.data_dir/filename);n=meta['counts'][key]
        if obj['Jv_samples'].shape[:2]!=(n,meta['counts']['sensitivity_mc']):raise ValueError('Response budget mismatch')
        if obj['x0'].shape!=(n,1,64,64) or obj['a'].shape!=(n,1):raise ValueError('Response condition shape mismatch')
        eps=float(obj['eps_a'])
        if not math.isfinite(eps) or eps<=0:raise ValueError('Invalid stored FD epsilon')
        out['x0'].append(obj['x0'].float());out['a'].append(obj['a'].float());out['eps'].append(torch.full((n,),eps))
...
def prepare(args):
    ...
    conditions=local_conditions(args)
    total=protocol['extra_trajectories'];mc=metadata(args)['counts']['sensitivity_mc'];chunks=[]
    pairs=total//2
    print(f'LocalBudget: {total} new simulator trajectories, matching the response-label trajectory count',flush=True)
    for start in range(0,pairs,args.sim_batch):
        count=min(args.sim_batch,pairs-start);path=directory/f'chunk_{start:07d}.pt'
        ids=torch.arange(start,start+count)//mc
        x=conditions['x0'][ids,0].to(sim.device);a=conditions['a'][ids,0].to(sim.device)
        eps=conditions['eps'][ids].to(sim.device)
        if path.exists():row=base.safe_load(path)
        else:
            generator.set_seed(GEN_SEED+start)
            rng=generator.capture_rng_state(sim.device)
            plus=sim.endpoint(x,a+eps)
            generator.restore_rng_state(rng,sim.device)
            minus=sim.endpoint(x,a-eps)
            finite(plus);finite(minus)
            row=dict(x0=torch.cat([x,x]).cpu().unsqueeze(1),
                     xT=torch.cat([plus,minus]).cpu().unsqueeze(1),
                     a=torch.cat([a+eps,a-eps]).cpu().reshape(-1,1))
            atomic_save(row,path)
        if row['a'].shape!=(2*count,1) or row['x0'].shape!=(2*count,1,sim.cfg.s,sim.cfg.s) or row['xT'].shape!=row['x0'].shape:
            raise ValueError(f'Invalid chunk {path}')
        torch.testing.assert_close(row['a'],torch.cat([a+eps,a-eps]).cpu().reshape(-1,1),rtol=0,atol=0)
        for value in row.values():finite(value)
        chunks.append(row);print(f'Local endpoint trajectories {2*(start+count)}/{total}',flush=True)
...
def train(args):
    ...
    protocol=dict(method='SNS LocalBudget',seed=args.seed,config=asdict(cfg),fork_sha256=sha(path),
...
def evaluate(args):
    output=args.run_root/'FINAL2_local_budget_mc16.json'
    ...
    save_json(output,dict(method='SNS LocalBudget',seeds=SEEDS,per_seed=per,aggregate=agg,
...
def main():
    ...
    p.add_argument('--run-root',type=Path,default=Path('runs/sns_local_budget_v1'))
