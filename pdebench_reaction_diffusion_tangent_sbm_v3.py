...
def main():
    ...
    anchor=load_response(data_dir/"anchor_response.pt"); colloc=load_response(data_dir/"response_collocation.pt")
    ...
    payload={"seed":args.seed,"lambda_sens":args.lambda_sens,"fork_imf":args.fork_imf,
             "total_imf":args.total_imf,"train_seconds":time.time()-start,"metrics":results}
    json.dump(payload,open(run_dir/"metrics.json","w"),indent=2)
    rows=[{"seed":args.seed,"lambda_sens":args.lambda_sens,"split":s,**m} for s,m in results.items()]
