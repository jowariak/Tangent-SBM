#!/usr/bin/env python3
"""
spdebench_sns_conditional_dsbm.py

Conditional DSBM baseline for the stochastic 2D Navier--Stokes response
benchmark generated by spdebench_sns_response_dataset.py.

The learned forward/backward drifts are conditioned on

    (vorticity field x, log2-viscosity intervention a, bridge time t),

but receive NO J* response supervision during training.

This script:
  * loads endpoint_train.pt;
  * scalar-normalizes vorticity using metadata.json;
  * trains ordinary conditional DSBM / IMF;
  * saves every IMF checkpoint;
  * evaluates endpoint mean + full endpoint distribution;
  * reports spread, energy distance, sliced Wasserstein, enstrophy error,
    response E_J, and finite-response error;
  * uses Jv_star only for evaluation, never for training.

The physical Q-Wiener realization is latent, so the learned target is the
stochastic conditional law p(x_T | x_0, a).
"""
...
class ConditionalUNetDrift(nn.Module):
    """1 state channel + 1 intervention + bridge time = 3 input channels."""
