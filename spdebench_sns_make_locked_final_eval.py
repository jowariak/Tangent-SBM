...
SPLITS = {
    "final_seen": [-1.0, 0.0, 1.0],
    "final_id": [-0.75, -0.25, 0.25, 0.75],
    "final_ood_near": [-1.25, 1.25],
    "final_ood_far": [-1.50, 1.50],
}
...
def main():
    ...
    manifest_path = data_dir / "final_eval_manifest.json"

    if manifest_path.exists() and not args.overwrite:
        raise RuntimeError(
            f"{manifest_path} already exists. The locked final set has already "
    ...
    print("LOCKED final evaluation seed =", args.seed, flush=True)
    print("fresh final ICs =", args.final_ics, flush=True)
    ...
    for split, a_values in SPLITS.items():
        ...
        obj = {
            ...
            "provenance": {
                ...
                "purpose": "single_locked_final_evaluation_after_all_hyperparameters_frozen",
            },
        }
    ...
    print("\nLOCKED MANIFEST SAVED:", manifest_path, flush=True)
