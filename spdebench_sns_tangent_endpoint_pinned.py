...
class TangentFieldDSBM(base.ConditionalFieldDSBM):
    def __init__(
        ...
        self.endpoint_generator = torch.Generator(device="cpu").manual_seed(int(sens_seed) + 1900003)
    ...
    @torch.no_grad()
    def _detached_endpoint(self, x0, a):
        noise = [torch.randn(
            x0.shape, generator=self.endpoint_generator,
            dtype=x0.dtype, device="cpu",
        ).to(x0.device) for _ in range(self.cfg.num_steps)]
        return self.sample_sde(x0, a, fb="f", noise_bank=noise).detach()
    ...
    def _pinned_states(self, x0, endpoint, noise_bank):
        ...
        for k in range(n):
            ...
            if remaining == 1:
                ...
                x = (x + (endpoint - x) / remaining
                     + self.cfg.reference_sigma
                     * math.sqrt(dt * (remaining - 1) / remaining)
                     * noise_bank[k])
    ...
    def tangent_training_rollout(self, x0, a, direction, noise_bank):
        ...
        for k, x in enumerate(states[:-1]):
            t = x.new_full((x.shape[0], 1), k * dt)

            def drift_fn(xi, ai):
                return self.net_f(xi, ai, t)

            _, tangent_drift = torch.autograd.functional.jvp(
                drift_fn, (x, a), (R, direction),
                create_graph=True, strict=False,
            )
...
def gradient_sanity(model, anchor, colloc):
    ...
    endpoint_state = model.endpoint_generator.get_state()
    ...
    try:
        ...
        model.endpoint_generator.set_state(endpoint_state)
...
def parse_args():
    ...
    p.add_argument(
        "--run-root",
        type=Path,
        default=Path("runs/spdebench_sns_endpoint_pinned"),
    )
...
def main():
    ...
    for imf in range(args.fork_imf + 1, args.total_imf + 1):
        ...
        save_obj = {
            ...
            "endpoint_rng_state": model.endpoint_generator.get_state(),
    ...
    training_summary = {
        "method": "tangent_sbm_endpoint_pinned",
