...
def subset_collocation(src, fraction):
    n = src["x0"].shape[0]
    if n < 1:
    ...
    indices = (torch.arange(n) if fraction == 1.0
               else torch.randperm(n, generator=generator)[:count])
    return {k: v[indices] for k, v in src.items()}, {
        "collocation_fraction": fraction, "available_collocation": n,
        "selected_collocation": count, "selected_indices": indices.tolist(),
        "subset_seed": 19092028, "anchor_fraction_retained": 1.0,
    }
...
def parse_args():
    ...
    p.add_argument(
        "--run-root",
        type=Path,
        default=Path("runs/sns_response_amount/production"),
    )
    p.add_argument("--collocation-fraction", type=float, choices=[0.25, 0.5, 1.0], required=True)
...
def main():
    ...
    colloc, subset_manifest = subset_collocation(colloc, args.collocation_fraction)
    subset_manifest["anchor_records"] = int(anchor["x0"].shape[0])
    ...
    run_dir = (
        args.run_root
        / f"colloc_{int(args.collocation_fraction * 100)}pct"
        / f"lambda_{lambda_tag(args.lambda_sens)}"
        / f"seed_{args.seed}"
    )
    if run_dir.exists() and any(run_dir.iterdir()):
        raise FileExistsError(f"Existing run: {run_dir}")
    run_dir.mkdir(parents=True, exist_ok=True)
    (run_dir / "response_subset.json").write_text(json.dumps(subset_manifest, indent=2))
    ...
    training_summary = {
        "method": "tangent_sbm_collocation_fraction",
